...
class RenkoEngine:
    ...
    def process_tick(self, tick):
        ...
        close_type, qty_bricks = self._check_brick_close(tick, self.brick_size)

        if close_type != "NO_CLOSE" and qty_bricks > 0:
            if qty_bricks == 1:
                
                closed_brick = self._build_closed_brick(close_type, tick["timestamp_ms"])
                if closed_brick:
                    self.repository.save_brick(self.symbol, closed_brick)
                    
                    self._reset_state_after_close(tick, closed_brick["close"])
            else:
                
                closed_bricks = self._build_closed_bricks(close_type, qty_bricks, tick)
                if closed_bricks:
                    for brick in closed_bricks:
                        self.repository.save_brick(self.symbol, brick)
                    
                    self._reset_state_after_close(tick, closed_bricks[-1]["close"])
            return  
    ...
    def _check_brick_close(self, tick, brick_size):
        ...
        last_brick = self.repository.get_last_closed_brick(
            self.symbol,
            brick_size
        ) 

        brick_current = self.repository.get_state(
            self.symbol,
            brick_size
        ) 
    ...
    def _build_closed_brick(
        ...
        if close_type == "REVERSAL_UP":
            direction = "UP"
            open_price = float(last_brick["open"])
            close_price = open_price + brick_distance - tick_size
            high_price = close_price
            low_price = min(open_price, close_price, self.state.low)
            ...
            open_price = float(last_brick["open"])
            close_price = open_price - (brick_distance - tick_size) 
            high_price = max(open_price, close_price, self.state.high)
            low_price = close_price

        if close_type == "CONTINUATION_UP":
            direction = "UP"
            open_price = float(last_brick["close"])
            close_price = open_price + brick_distance - tick_size
            high_price = close_price
            low_price = min(open_price, close_price, self.state.low)
    ...
    def _build_closed_bricks(
        ...
        # FIRST BRICK
        first_close_time = self.state.open_time + step if qty_bricks > 1 else final_close_time
        first_brick = self._build_closed_brick(close_type, first_close_time)
        if first_brick:
    ...
    def _reset_state_after_close(self, tick, open_price):
        ...
        state.high = max(open_price, tick["last"])
        state.low = min(open_price, tick["last"])
